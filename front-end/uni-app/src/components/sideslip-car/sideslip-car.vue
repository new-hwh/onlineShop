<template>
	<view class="car-count">
		<view class="cart-box" v-if='carts.length>0'>
			<view class="touch-item" v-for='(item,index) in carts' :class="item.isTouchMove == true?'touch-move-active':''" :key='item.goods.id'
				...
				<view class="cartList clearfix">
					...
					<view class="gmes">
						<view class='cartImg centerboth'>
							<image :src="item.goods.show_pic || emptyImg" mode="aspectFit"></image>
						</view>
						<view class="boxCount clearfix">
							<text class="gname line1">{{item.goods.name}}</text>
							<view class="gprice"><text>￥</text>{{item.goods.unit_code}}</view>
						</view>
					</view>
					<view class="countBox centerboth">
						...
						<view class="cartNum">{{item.goods_num}}</view>
	...
	export default {
		...
		methods: {
			...
			changeNum:function(index,id,type) {
				...
				var number = parseInt(carts[index].goods_num);
				if(type==0){
					number = number - 1;
					if(number<=1){
						number = 1;
				...
				carts[index].goods_num = number;
				this.$emit('changeNum',carts,id,number);
			},
			/**
			 * 删除购物车当前商品
			 */
			deleteList(index,id) {
				...
				this.$emit('delBtn',carts,id,index)
			},
			/**
			 * 当前商品选中事件
			 */
			...
			getTotalPrice:function() {
				...
				for (var i = 0; i < carts.length; i++) { // 循环列表得到每个数据
					if (carts[i].selected) { // 判断选中才会计算价格
						total += carts[i].goods_num * carts[i].goods.unit_code; // 所有价格加起来
						totalnum = totalnum + parseInt(carts[i].goods_num);
					}
				}
